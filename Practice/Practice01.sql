...
하세요.
*/
/***잘모르겠음 업무별로 최고월급을 모아놓은 거니까 내림차순 정렬할거면 최고월급을 정렬하는*/
select job_title,
       max_salary 
from jobs
...
select *
from departments
order by length(department_name) desc; --큰값이 알파벳z로 인식되는데 길이는 어떻게 표현하는가 -> length 함수 사용함
